 ...
 * Xxl-job 客户端模板
 * <p>
 * 通过模板方法,在每次调用命令前都去登录,然后获取cookie，通过cookie模拟登录,然后再发起正在的命令调用。因为okhttp用了连接池的，所以几乎不用考虑性能问题
 * <p>
 * ps：为什么都要登录？因为控制台用了拦截器的，所以必须要登录？
 * ps：为什么每次都需要登录，难道不可以登录一次缓存cookie吗？其实完全可以这样的，但是如果登录过期了，再访问的时候，直接跳转到了登录界面，
 * 在请求方是无法判断是否重定向的，只能通过内容判断是不是未登录，我个人觉得很麻烦，索性每次登录算了
 * <p>
 * 扩展：还有另一种方法，通过okhttp的回调机制，在回调方法中去 触发命令调用，这个留个读者自行扩展
 *
 * @Author: FengJie
